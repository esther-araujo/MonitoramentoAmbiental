...
void getHistorico(){
    for(int i=0;i<historicoQtd;i++){
        sprintf(historico, "%.2f|%.2f|%.2f|%.2f|%s %s", 
            temperaturaH[i], luminosidadeH[i], umidadeH[i], pressaoH[i], dataH[i], horaH[i]);
    }
}
...
void printHistorico(){
    ...
    //resetLcd(lcd);
    //lcdPrintf(lcd,"%s", dataH[historicoIndex]);
    //lcdPosition(lcd, 0, 1);
    //lcdPrintf(lcd, "%s", horaH[historicoIndex] );
    printf("datatime: %s %s\n", dataH[historicoIndex], horaH[historicoIndex]);
}
...
void updateHistorico(){
    ...
    for(int i=0;i<historicoQtd-1;i++){
        ...
        strcat(dataH[i+1], dataH[i]);
        strcat(horaH[i+1], horaH[i]);
    }
    temperaturaH[0] = temperatura; 
    umidadeH[0] = umidade; 
    pressaoH[0] = pressao; 
    luminosidadeH[0] = luminosidade; 
    sprintf(dataH[0], "%02d/%02d/%d", tempo0->tm_mday, tempo0->tm_mon, tempo0->tm_year);
